...
export function NetworkPage(){
  ...
  const totalSpeed=upIfaces.reduce(
    (a,n)=>a+n.speedMbps,
    0
  );

  const totalRxGB=interfaces.reduce(
    (a,n)=>a+n.rxBytes,
    0
  );

  const totalTxGB=interfaces.reduce(
    (a,n)=>a+n.txBytes,
    0
  );
  ...
  return(
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-4">
        <Stat label="Téléchargement" value={fmtMbps(live.sys.netRxMbps)} sub={`Total ${live.sys.netRxTotalGB} GB`} tone="text-brand-300"/>
        <Stat label="Envoi" value={fmtMbps(live.sys.netTxMbps)} sub={`Total ${live.sys.netTxTotalGB} GB`} tone="text-accent-400"/>
        <Stat label="Interfaces actives" value={`${upIfaces.length}/${interfaces.length}`} sub="Interfaces UP" tone="text-ink-100"/>
        <Stat label="Vitesse cumulée" value={`${totalSpeed} Mbps`} sub={networkUpdatedAt?`API ${networkUpdatedAt}`:'En attente'} tone="text-ink-100"/>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="card card-pad lg:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-ink-400">Débit instantané</p>
            </div>
          ...
          <div className="mt-4 space-y-5">
            <div>
              ...
              <Sparkline data={live.netRxHistory} color="#10b981" height={60} max={650}/>
            </div>
            ...
            <div>
              ...
              <Sparkline data={live.netTxHistory} color="#0ea5e9" height={60} max={350}/>
            </div>
          </div>
        </div>
        ...
        <div className="card card-pad">
          ...
          <div className="mt-4 space-y-3">
            <Total label="Reçu" value={fmtGB(totalRxGB)} tone="text-brand-300"/>
            <Total label="Émis" value={fmtGB(totalTxGB)} tone="text-accent-400"/>
            <Total label="Vitesse" value={`${totalSpeed} Mbps`} tone="text-ink-100"/>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {interfaces.map(n=>(
                <tr key={n.name} className="border-b border-ink-800/60">
                  ...
                  <td className="px-4 py-3 text-right">{n.speedMbps} Mbps</td>
                  <td className="px-4 py-3 text-right text-brand-300">{fmtGB(n.rxBytes)}</td>
                  <td className="px-4 py-3 text-right text-accent-400">{fmtGB(n.txBytes)}</td>
                </tr>
              ))}
            </tbody>
          </table>
